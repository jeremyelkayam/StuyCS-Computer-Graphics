  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      index = lat * (num_steps+1) + longt;
			...
			index1 = (lat+1) * (num_steps+1) + longt - 1;
			index2 = (lat+1) * (num_steps+1) + longt;
			index3 = lat * (num_steps+1) + longt + 1;

			add_polygon(points, temp->m[0][index], temp->m[1][index], temp->m[2][index],
				temp->m[0][index1], temp->m[1][index1], temp->m[2][index1],
				temp->m[0][index2], temp->m[1][index2], temp->m[2][index2] );
			add_polygon(points, temp->m[0][index], temp->m[1][index], temp->m[2][index],
				temp->m[0][index2], temp->m[1][index2], temp->m[2][index2],
				temp->m[0][index3], temp->m[1][index3], temp->m[2][index3] );
    }//end points only
  }
}
