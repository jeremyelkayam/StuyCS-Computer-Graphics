...
void first_pass() {
  ...
  for (x = 0; x < lastop; x++){
    ...
    case FRAMES:
      if (frames){
        printf("Already set frame, this can only be done once\n");
        exit(0);
      } else {
    ...
    case BASENAME:
      if (basename){
        printf("Already set basename, this can only be done once\n");
        exit(0);
      } else {
        strncpy(name, op[x].op.basename.p->name, strlen(op[x].op.basename.p->name));
        basename = 1;
    ...
    case VARY:
      if (!frames){
        printf("Need to set frame first\n");
        exit(0);
      }
      break;
    }
  }
  if ((frames || vary) && !basename){
    strcpy(name, "animation");
    printf("No basename specified, default name is %s\n", name);
  }
}
...
struct vary_node ** second_pass() {
  ...
  struct vary_node * k_node;
  for (x = 0; x < lastop; x++){
    if (op[x].opcode == VARY){
      int y;
      start_f = op[x].op.vary.start_frame;
      end_f = op[x].op.vary.end_frame;
      v = op[x].op.vary.start_val;
      end_v = op[x].op.vary.end_val;
      //Some kinda bug around here
      inc = (end_v - v) / (end_f - start_f);
      for (y = start_f; y <= end_f; y++){
        k_node = (struct vary_node *)malloc(sizeof(struct vary_node)); 
        strncpy(k_node->name, op[x].op.vary.p->name, sizeof(k_node->name));
        k_node->value = v;
        k_node->next = knobs[y];
        knobs[y] = k_node;
        v += inc;
...
void my_main( int polygons ) {
  ...
  for (f = 0; f < num_frames; f++){
	...
	break;
      ...
      if (num_frames!=1){
      	sprintf (frame_name, "./animation/%s%03d.png", name, f );
